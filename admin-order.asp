...
If (CStr(Request("MM_delete")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
    ...
    MM_editCmd.CommandText = "Delete from dbo.tb_order where orderID = ?"
    MM_editCmd.Parameters.Append MM_editCmd.CreateParameter("param1", 5, 1, -1, Request.Form("MM_recordId"))
    MM_editCmd.Execute
    MM_editCmd.ActiveConnection.Close

    ' append the query string to the redirect URL
    Dim MM_editRedirectUrls
    MM_editRedirectUrl = "admin-order.asp"
    If (Request.QueryString <> "") Then
      ...
      End If
    End If
	Session("statusDelproduct") = "Xóa thành công đơn hàng có ID = "&Request.Form("MM_recordId")
...
if (Request.QueryString <> "") then
	...
	if Request.QueryString("status") <> "" then
		querylistproductPlus = "WHERE status = "&status
	else 
...
rsListOrder_cmd.CommandText = "SELECT * FROM dbo.tb_order as od left join dbo.tb_user as us on od.userID = us.userID "&querylistproductPlus&" ORDER BY dateOrder DESC"
                    ...
                    <tbody>
						...
						<%
							...
							if statusorder = "True" then
								statusorder = "<a href=""?confirmOrder="&(rsListOrder.Fields.Item("orderID").Value)&"&valueConfirm=0"" class=""btn btn-primary"">Đã xử lý</a>"
							else
								statusorder = "<a href=""?confirmOrder="&(rsListOrder.Fields.Item("orderID").Value)&"&valueConfirm=1"" class=""btn btn-primary"">Chưa xử lý</a>"
							end if
						%>
                      <tr>
                        <td><a href="user-info.asp?email=<%=rsListOrder.Fields.Item("email").Value%>"><%=(rsListOrder.Fields.Item("fullName").Value)%></a></td>
                        ...
                        <td><%=statusorder%></td>
                        <td>
						<button data-toggle="modal" data-target="#orderID-<%=(rsListOrder.Fields.Item("orderID").Value)%>" type="submit" class="btn btn-primary" style="width: 80px;">Xem</button>
                              ...
                              <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-footer">
                                    ...
                                    <a href="?confirmOrder=<%=(rsListOrder.Fields.Item("orderID").Value)%>&valueConfirm=1" class="btn btn-primary">Xác nhận</a>
                                  </div>
                                </div>
                              </div>
                            </div>
                        ...
                        <input type="hidden" name="MM_recordId" value="<%=rsListOrder.Fields.Item("orderID").Value %>">
                        </form>
                        </td><% 
